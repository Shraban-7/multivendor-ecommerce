...
<style>
    ...
    .perf-history__spark-bar  { position: absolute; top: 0; bottom: 0; left: 0; border-radius: 999px; }
</style>
@endpush
...
<section class="bg-white rounded-sm shadow-sm overflow-hidden mb-4 relative">
    ...
    <div class="p-5 lg:p-6 pt-6">
        <div class="flex flex-wrap items-start justify-between gap-4">
            <div class="min-w-0">
                <nav class="flex items-center gap-1 mb-2 text-xs text-ink-tertiary">
                    ...
                    <a href="{{ route('seller.performance.dashboard') }}" class="hover:text-ink-soft transition-colors">Performance</a>
                    <i data-lucide="chevron-right" style="width:12px;height:12px;"></i>
                    <span class="text-ink-soft font-semibold">History</span>
                </nav>
                <h1 class="text-xl font-bold text-ink-emphasis mb-1">Performance History</h1>
                <p class="text-sm text-ink-secondary mb-0">
                    Daily snapshots from the last <strong class="text-ink-emphasis">{{ $days }}</strong> days ·
                    <strong class="text-ink-emphasis">{{ $totalSnapshots }}</strong> {{ Str::plural('record', $totalSnapshots) }}
                </p>
            </div>
        ...
        <div class="mt-4 flex flex-wrap gap-2 text-sm items-center text-ink-secondary">
            ...
            <span class="font-medium text-ink-emphasis me-1">Range:</span>
            @foreach ([30, 60, 90, 180, 365] as $d)
                <a href="{{ route('seller.performance.history', ['days' => $d]) }}"
                   class="px-2 py-0.5 rounded-xs transition-colors {{ $days === $d ? 'bg-brand-tint text-brand-deep font-semibold' : 'hover:bg-surface-muted text-ink-secondary' }}">
                    Last {{ $d }}d
                </a>
            @endforeach
        </div>
...
<section class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3 mb-4">
    @foreach ($historyKpis as $kpi)
        <article class="bg-white rounded-sm shadow-sm p-4 transition-shadow hover:shadow-md relative overflow-hidden">
            <div class="h-1 absolute top-0 left-0 right-0 {{ $tonBg[$kpi['tone']] ?? 'bg-gray-500' }}"></div>
...
<section class="grid grid-cols-1 xl:grid-cols-3 gap-3 mb-4">
    <div class="bg-white rounded-sm shadow-sm overflow-hidden">
        <div class="px-5 py-3 flex items-center gap-2">
            ...
            <h5 class="mb-0 font-bold text-ink-emphasis text-sm">Tier Distribution</h5>
        </div>
                    ...
                    <div>
                        <div class="flex items-center justify-between text-xs mb-1">
                            <span class="flex items-center gap-2">
                                ...
                                <strong class="text-ink-emphasis font-semibold">{{ $tierLabel }}</strong>
                            </span>
    ...
    <div class="xl:col-span-2 bg-white rounded-sm shadow-sm overflow-hidden">
        <div class="px-5 py-3 flex items-center justify-between">
            <div class="flex items-center gap-2">
                ...
                <h5 class="mb-0 font-bold text-ink-emphasis text-sm">Score over Time</h5>
            </div>
...
<section class="bg-white rounded-sm shadow-sm overflow-hidden">
    <div class="px-5 py-3 flex items-center justify-between">
        <div class="flex items-center gap-2">
            ...
            <h5 class="mb-0 font-bold text-ink-emphasis text-sm">Daily Snapshots</h5>
        </div>
            ...
            <tbody class="divide-y divide-border">
                @forelse ($snapshots->sortByDesc('snapshot_date')->values() as $snapshot)
                    @php
                        ...
                        $sBadge = $sEnum ? $tierBadge[$sEnum->value] : 'bg-surface-muted text-ink-tertiary';
                        $scoreVal = (float) $snapshot->overall_score;
                    @endphp
                    <tr class="hover:bg-surface-muted/50 transition-colors">
                        <td class="px-4 py-3 font-mono text-ink-emphasis">{{ $snapshot->snapshot_date->format('d M Y') }}<br><small class="text-ink-tertiary">{{ $snapshot->snapshot_date->format('D') }}</small></td>
                        <td class="px-4 py-3 text-right">
                            <span class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-semibold bg-info-tint text-feedback-info">{{ number_format($snapshot->total_orders) }}</span>
                        </td>
                        <td class="px-4 py-3 text-right font-semibold text-ink-emphasis">{{ number_format($snapshot->delivered_orders) }}</td>
                        <td class="px-4 py-3 text-right">
                            @if ((float) $snapshot->late_shipping_rate >= 0.30)
                                <span class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-semibold text-white bg-rose-500">{{ number_format($snapshot->late_shipped_orders) }}</span>
                            @elseif ((float) $snapshot->late_shipping_rate >= 0.10)
                                <span class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-semibold text-white bg-amber-500">{{ number_format($snapshot->late_shipped_orders) }}</span>
                            @else
                                <span class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-semibold text-white bg-emerald-500">{{ number_format($snapshot->late_shipped_orders) }}</span>
                            @endif
                            <br><small class="text-ink-tertiary">{{ round($snapshot->late_shipping_rate * 100, 1) }}%</small>
                        </td>
                        <td class="px-4 py-3 text-right">
                            @if ((float) $snapshot->cancellation_rate >= 0.10)
                                <span class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-semibold text-white bg-rose-500">{{ number_format($snapshot->cancelled_orders) }}</span>
                            @else
                        ...
                        <td class="px-4 py-3 text-right font-semibold text-ink-emphasis">{{ number_format($snapshot->review_count) }}</td>
                        <td class="px-4 py-3 text-right">
                            ...
                            @elseif ((float) $snapshot->avg_review_rating > 0)
                                <span class="text-feedback-warning font-semibold">{{ number_format((float) $snapshot->avg_review_rating, 2) }}</span>
                            @else
                        ...
                        <td class="px-4 py-3">
                            <span class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium text-white {{ $sBadge }}">{{ $sLabel }}</span>
                        </td>
                        <td class="px-4 py-3" style="min-width: 80px;">
                            <div class="perf-history__spark-track" style="width: 60px; height: 6px;">
                ...
                @empty
                    <tr>
                        <td colspan="10" class="text-center py-10 text-sm text-ink-tertiary">
                            <i data-lucide="history" class="mx-auto mb-3 opacity-50" style="width:40px;height:40px;"></i>
                            <p class="mb-1 font-semibold text-ink-emphasis">No history snapshots yet</p>
