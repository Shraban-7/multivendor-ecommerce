...
@section('content')
                ...
                <div class="min-w-0">
                    <nav class="flex items-center gap-1 mb-2 text-xs text-ink-tertiary">
                        ...
                        <a href="{{ route('seller.payouts.index') }}"
                            class="hover:text-ink-soft transition-colors">Payouts</a>
                    ...
                    <p class="text-sm text-ink-secondary mb-0">Withdraw funds from your available balance to your registered
                        payout method.</p>
                </div>
                <div>
    ...
    <section
        class="bg-gradient-to-r from-orange-500 to-orange-400 rounded-sm shadow-sm overflow-hidden mb-4 text-white relative">
        <div class="px-5 py-4 flex flex-wrap items-center justify-between gap-3">
            <div class="flex items-center gap-3">
                ...
                <div>
                    <p class="text-xs uppercase tracking-wider opacity-75 mb-0 font-semibold text-white/90">Available
                        Balance</p>
                ...
                <form method="POST" action="{{ route('seller.payouts.store') }}">
                    ...
                    <div class="p-5 space-y-4">
                        ...
                        <div>
                            ...
                            <div
                                class="flex bg-surface-muted rounded-xs overflow-hidden focus-within:ring-1 focus-within:ring-brand-deep">
                                <span
                                    class="inline-flex items-center px-3 text-ink-tertiary font-semibold text-sm">{{ currency('symbol') }}</span>
                            ...
                            <div class="flex flex-wrap items-center gap-2 mt-1">
                                <small class="text-ink-tertiary">Minimum {{ money($feeConfig['minimum_withdrawal'] ?? 1) }}
                                    — Maximum <span
                                        class="font-semibold text-ink-soft">{{ money($availableBalance ?? 0) }}</span></small>
                                <div class="flex gap-1 ml-auto">
                                    @foreach ([25, 50, 75, 100] as $pct)
                                        <button type="button"
                                            class="preset-amt px-2 py-0.5 text-[11px] font-semibold rounded-full bg-surface-muted text-ink-soft hover:bg-brand-tint hover:text-brand-deep transition-colors"
                        ...
                        <div>
                            ...
                            @if ($methods && $methods->count() > 0)
                                <select name="payout_method_id"
                                    ...
                                    @foreach ($methods as $method)
                                        <option value="{{ $method->id }}"
                                            {{ old('payout_method_id', optional($methods->where('is_default', true))->first()->id) == $method->id ? 'selected' : '' }}>
                                ...
                                @error('payout_method_id')
                                    <div class="invalid-feedback block mt-1 text-xs text-feedback-danger">{{ $message }}
                                    </div>
                                @enderror
                                <small class="text-ink-tertiary mt-1 block">
                                    Need a different method?
                                    <a href="{{ route('seller.payouts.methods.index') }}"
                                        class="text-brand hover:text-brand-deep font-semibold">Manage payment methods</a>
                                </small>
                            @else
                                <div class="flex items-start gap-2 p-4 rounded-xs bg-amber-50 text-ink-soft">
                                    <i data-lucide="alert-triangle" class="text-feedback-warning shrink-0 mt-0.5"
                                        style="width:16px;height:16px;"></i>
                                    <div class="text-sm">
                                        <p class="mb-1 font-semibold text-ink-emphasis"><strong>No payout methods
                                                found.</strong></p>
                                        <p class="mb-0 text-ink-secondary">Add a payout method first, then come back to
                                            withdraw.</p>
                                    </div>
                                    <a href="{{ route('seller.payouts.methods.index') }}"
                                        class="btn btn-warning btn-sm ms-auto shrink-0">
                        ...
                        {{-- Note --}}
                        <div>
                            <label class="block text-xs font-semibold text-ink-secondary mb-1 uppercase tracking-wider">Note
                                <span class="text-ink-tertiary font-normal normal-case">(Optional)</span></label>
                            <textarea name="seller_note" rows="3" placeholder="Any additional information for the admin team…"
                        ...
                        <div class="rounded-xs bg-surface-muted overflow-hidden">
                            <div class="px-4 py-2 bg-brand-tint flex items-center gap-2">
                                ...
                                <span class="text-xs font-bold text-brand-deep uppercase tracking-wider">Fee
                                    Breakdown</span>
                            ...
                            <div class="p-4 space-y-2">
                                <div class="flex justify-between text-sm">
                                    ...
                                    <span class="font-semibold text-ink-emphasis"
                                        id="previewAmount">{{ currency('symbol') }}0.00</span>
                                </div>
                                <div class="flex justify-between text-sm">
                                    <span class="text-ink-secondary">Processing fee</span>
                                    <span class="font-semibold text-feedback-danger" id="previewFee">−
                                ...
                                <div class="border-t border-border pt-2 mt-2 flex justify-between items-center">
                                    ...
                                    <span class="font-bold text-2xl text-feedback-success"
                                        id="previewNet">{{ currency('symbol') }}0.00</span>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="px-5 py-3 bg-surface-muted flex flex-wrap items-center justify-end gap-2">
                        ...
                        <button type="submit" class="btn btn-primary"
                            {{ !$methods || $methods->isEmpty() ? 'disabled' : '' }}>
            ...
            <section class="bg-white rounded-sm shadow-sm overflow-hidden">
                ...
                <div class="p-5">
                    <ul class="space-y-3 mb-0">
                        <li class="flex gap-3">
                            <span
                                class="shrink-0 w-7 h-7 rounded-sm bg-brand-tint text-brand-deep flex items-center justify-center">
                            ...
                            <div>
                                ...
                                <small class="text-ink-tertiary">{{ money($feeConfig['minimum_withdrawal'] ?? 1) }} per
                                    request</small>
                            </div>
                        </li>
                        <li class="flex gap-3">
                            <span
                                class="shrink-0 w-7 h-7 rounded-sm bg-warning-tint text-feedback-warning flex items-center justify-center">
                                ...
                                <small class="text-ink-tertiary">
                                    ...
                                    @else
                                        @foreach ($tiers as $i => $tier)
                                            ...
                                            @endif
                                            {{ $tier['rate'] }}% for
                                            {{ $i === 0 ? '≥ ' . number_format($tier['min']) : number_format($tier['min']) . '–' . number_format(($tiers[$i - 1]['min'] ?? 0) - 1) }}
                                        @endforeach
                        ...
                        <li class="flex gap-3">
                            <span
                                class="shrink-0 w-7 h-7 rounded-sm bg-info-tint text-feedback-info flex items-center justify-center">
                        ...
                        <li class="flex gap-3">
                            <span
                                class="shrink-0 w-7 h-7 rounded-sm bg-emerald-50 text-feedback-success flex items-center justify-center">
                    ...
                    <div class="mt-4 p-3 rounded-xs bg-emerald-50">
                        <p class="mb-0 flex items-start gap-2 text-xs text-ink-soft">
                            <i data-lucide="shield-check" class="text-feedback-success shrink-0 mt-0.5"
                                style="width:14px;height:14px;"></i>
                            <span>Once submitted, your request will be reviewed by our team. We'll notify you when it's
                                approved or rejected.</span>
                        </p>
                    </div>
                </div>
            </section>
