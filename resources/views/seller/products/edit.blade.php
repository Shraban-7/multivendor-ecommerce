            ...
            <form id="form" enctype="multipart/form-data" method="POST">
                <div class="row">
                    ...
                    <div class="col-md-4 mb-3">
                        ...
                        <select name="subcategory_id" class="form-select w-100" id="subcategorySelect" {{ $product->subcategory_id ? '' : 'disabled' }}>
                            <option value="" disabled>--Choose--</option>
                            @foreach ($categories as $category)
                                @foreach ($category->subcategories as $subcategory)
                                    <option value="{{ $subcategory->id }}" data-category="{{ $category->id }}" 
                                        {{ $subcategory->id == $product->subcategory_id ? 'selected' : '' }}>{{ $subcategory->name }}
                                    </option>
                                @endforeach
                            @endforeach
                        </select>
                    ...
                    <div class="col-md-4 mb-3">
                        ...
                        <input name="name" type="text" value="{{ old('name', $product->name) }}" class="form-control" required> 
                    </div>
                    <div class="col-md-4 mb-3">
                        <label class="form-label">Slug</label>
                        <input name="slug" type="text" value="{{ old('slug', $product->slug) }}" class="form-control" required> 
                    </div>
                    <div class="col-md-4 mb-3">
                        <label class="form-label">Sku</label>
                        <input name="sku" type="text" value="{{ old('sku', $product->sku) }}" class="form-control" required>
                    </div>
                    <div class="col-md-4 mb-3">
                        <label class="form-label">Buying Price</label>
                        <input name="buying_price" type="text" value="{{ old('buying_price', $product->buying_price) }}" class="form-control" required>
                    </div>
                    <div class="col-md-4 mb-3">
                        <label class="form-label">Selling Price</label>
                        <input name="selling_price" type="text" value="{{ old('selling_price', $product->selling_price) }}" class="form-control" required>
                    </div>

                    <div class="col-md-4 mb-3">
                        <label class="form-label">Current stock</label>
                        <input name="stock_in" type="text" value="{{ old('stock_in', $product->stock_in) }}" class="form-control" required>
                    </div>
                        ...
                        <div id="selectedImages" class="row mb-2">
                            @foreach ($product->images as $image)
                                <div class="col-2 mb-2">
                                    <img src="{{ asset('storage/' . $image->image) }}" alt="image" class="col-2" style="width: 100%; height: 150px;">
                                    <button type="button" class="btn btn-danger btn-sm mt-2" 
                                        style="width: 50%" onclick="deleteImage({{ $image->id }})">Delete
                                    </button>
                                </div>
                            @endforeach
                        </div>
