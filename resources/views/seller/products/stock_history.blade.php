...
<section class="bg-white rounded-sm shadow-sm overflow-hidden mb-3 relative">
    ...
    <div class="p-5 lg:p-6 pt-6">
        <div class="flex flex-wrap items-start justify-between gap-4">
            ...
            <div class="shrink-0">
                <button type="button" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#stockUpdateModal">
...
@php
    $tiles = [
        ['key' => '_total',   'label' => 'Total Entries',  'top' => '#06b6d4', 'text' => 'text-[#06b6d4]',         'icon' => 'history', 'value' => $totalCount],
        ['key' => '_added',   'label' => 'Stock Added',     'top' => '#10b981', 'text' => 'text-feedback-success',  'icon' => 'plus',    'value' => $addedCount, 'prefix' => '+'],
        ['key' => '_removed', 'label' => 'Stock Removed',   'top' => '#ef4444', 'text' => 'text-feedback-danger',   'icon' => 'minus',   'value' => $removedCount, 'prefix' => '-'],
        ['key' => '_set',     'label' => 'Set Exact',       'top' => '#B7791A', 'text' => 'text-feedback-warning',  'icon' => 'pencil',  'value' => $setCount],
    ];
@endphp
    ...
    @foreach ($tiles as $tile)
        <article class="bg-white rounded-sm shadow-sm overflow-hidden relative">
            ...
            <div class="p-4 pt-5">
                ...
                <h3 class="text-2xl font-bold {{ $tile['text'] }} mb-0">{{ ($tile['prefix'] ?? '') . number_format($tile['value']) }}</h3>
            </div>
        </article>
    @endforeach
</section>
...
<section class="bg-white rounded-sm shadow-sm overflow-hidden mb-3">
    <div class="px-5 py-3 bg-surface-muted flex items-center gap-2">
        ...
        @if(request('q') || request('type') || request('from') || request('to'))
            <a href="{{ route('seller.stock.index') }}" class="text-[11px] font-semibold text-ink-tertiary hover:text-ink-emphasis inline-flex items-center gap-1">
    ...
    <div class="overflow-x-auto px-4 pb-4">
        <table class="w-full text-left text-sm border-collapse">
            <thead class="bg-surface-muted">
                <tr>
                    <th class="px-4 py-3 text-[11px] font-semibold uppercase tracking-wider text-ink-tertiary">Product / Variant</th>
                    ...
                    <th class="px-4 py-3 text-[11px] font-semibold uppercase tracking-wider text-ink-tertiary text-center">Quantity</th>
                    ...
                    <tr class="border-t border-border hover:bg-surface-muted/40 transition-colors">
                        <td class="px-4 py-3">
                            <div class="flex items-center gap-2">
                                ...
                                <div class="min-w-0">
                                    <div class="font-semibold text-ink-emphasis text-sm truncate max-w-[200px]">{{ $history->product?->name ?? '—' }}</div>
                                    <small class="text-ink-tertiary">{{ $history->variant?->label ?: 'Simple product' }}</small>
                                </div>
                            </div>
                        </td>
                        <td class="px-4 py-3 text-xs">
                            <code class="px-1.5 py-0.5 rounded-xs bg-surface-muted text-ink-secondary">{{ $history->variant?->sku ?? $history->product?->sku ?? '—' }}</code>
                        </td>
                        <td class="px-4 py-3">
                            <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-[11px] font-bold uppercase tracking-wider {{ $pillBg }}">
                                {{ $history->type->value === StockType::ADD_STOCK->value ? 'Add' :
                                   ($history->type->value === StockType::REMOVE_STOCK->value ? 'Remove' :
                                   ($history->type->value === StockType::SET_EXACT_STOCK->value ? 'Set' : '—')) }}
                            </span>
                        </td>
                        <td class="px-4 py-3 text-center font-semibold
                            {{ $history->type === StockType::ADD_STOCK ? 'text-feedback-success' :
                               ($history->type === StockType::REMOVE_STOCK ? 'text-feedback-danger' : 'text-ink-emphasis') }}">
                            {{ $history->type === StockType::ADD_STOCK ? '+' . $history->quantity :
                               ($history->type === StockType::REMOVE_STOCK ? '-' . abs($history->quantity) : $history->quantity) }}
                        </td>
                    ...
                    <tr>
                        <td colspan="6">
                            <div class="py-10 text-center">
                                <i data-lucide="history" class="text-ink-tertiary mx-auto mb-2" style="width:36px;height:36px;"></i>
                                <p class="text-ink-soft font-semibold mb-1">No stock history yet</p>
                                <p class="text-ink-tertiary text-xs">Click <strong>Update Stock</strong> to record your first adjustment.</p>
                            </div>
                        </td>
                    </tr>
                @endforelse
        ...
        <div class="modal-content">
            <form id="stockForm" action="{{ route('seller.stock.update') }}" method="POST">
                ...
                <div class="modal-body">
                    <div class="mb-3">
                        <label for="productSelect" class="block text-[11px] font-semibold text-ink-tertiary mb-1 uppercase tracking-wider">Select Product <span class="text-feedback-danger">*</span></label>
                    ...
                    <div class="mb-3 d-none" id="variantContainer">
                        <label for="variantSelect" class="block text-[11px] font-semibold text-ink-tertiary mb-1 uppercase tracking-wider">Select Variant <span class="text-feedback-danger">*</span></label>
                    ...
                    <div class="mb-3" id="currentStockBox" style="display:none">
                        <div class="p-3 rounded-sm bg-surface-muted flex items-center justify-between">
                            <div>
                                <div class="text-[11px] text-ink-tertiary uppercase tracking-wider font-semibold mb-1">Current stock</div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-2">
                        <div>
                            <label for="stockQuantity" class="block text-[11px] font-semibold text-ink-tertiary mb-1 uppercase tracking-wider">Quantity <span class="text-feedback-danger">*</span></label>
                        ...
                        <div class="md:col-span-2">
                            <label class="block text-[11px] font-semibold text-ink-tertiary mb-2 uppercase tracking-wider">Stock Action <span class="text-feedback-danger">*</span></label>
                            <div class="flex gap-3 flex-wrap p-2 rounded-sm bg-surface-muted">
                                <div class="flex items-center gap-2">
                                    <input class="h-4 w-4 rounded text-brand focus:outline-none focus:ring-1 focus:ring-brand-deep" type="radio" name="stock_action" id="addStock" value="{{ StockType::ADD_STOCK->value }}" checked>
                                    <label class="flex items-center gap-1 text-sm text-ink-emphasis cursor-pointer" for="addStock"><span class="inline-block w-2 h-2 rounded-full bg-feedback-success"></span> {{ StockType::ADD_STOCK->label() }}</label>
                                </div>
                                <div class="flex items-center gap-2">
                                    <input class="h-4 w-4 rounded text-brand focus:outline-none focus:ring-1 focus:ring-brand-deep" type="radio" name="stock_action" id="removeStock" value="{{ StockType::REMOVE_STOCK->value }}">
                                    <label class="flex items-center gap-1 text-sm text-ink-emphasis cursor-pointer" for="removeStock"><span class="inline-block w-2 h-2 rounded-full bg-feedback-danger"></span> {{ StockType::REMOVE_STOCK->label() }}</label>
                                </div>
                                <div class="flex items-center gap-2">
                                    <input class="h-4 w-4 rounded text-brand focus:outline-none focus:ring-1 focus:ring-brand-deep" type="radio" name="stock_action" id="setStock" value="{{ StockType::SET_EXACT_STOCK->value }}">
                                    <label class="flex items-center gap-1 text-sm text-ink-emphasis cursor-pointer" for="setStock"><span class="inline-block w-2 h-2 rounded-full bg-feedback-warning"></span> {{ StockType::SET_EXACT_STOCK->label() }}</label>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div class="mt-3">
                        <label for="stockNote" class="block text-[11px] font-semibold text-ink-tertiary mb-1 uppercase tracking-wider">Note <span class="font-normal normal-case">(Optional)</span></label>
