...
<div class="grid grid-cols-1 gap-3">
    <div class="md:col-span-1">
        ...
        <input type="text" name="title"
            ...
            value="{{ old('title', $c->title ?? '') }}" placeholder="e.g., Summer Sale 2026">
    </div>
    <div class="md:col-span-1">
    ...
    <div class="md:col-span-1">
        ...
        <select name="discount_type"
            ...
            required>
            <option value="percentage" {{ $c && $c->discount_type === 'percentage' ? 'selected' : '' }}>Percentage (%)
            </option>
    ...
    <div class="md:col-span-1">
        ...
        <input type="date" name="valid_until"
            class="w-full px-3 py-2 text-sm text-ink bg-white border border-border rounded-xs focus:outline-none focus:border-brand-deep focus:ring-1 focus:ring-brand-deep placeholder:text-ink-tertiary transition-colors"
            value="{{ old('valid_until', $c ? $c->valid_until->format('Y-m-d') : now()->addMonth()->format('Y-m-d')) }}"
            required>
    </div>
    <div class="col-span-full">
        <label class="block text-xs font-medium text-ink-secondary mb-1">Applicable Products</label>
        <select id="applicableProducts" name="product_ids[]" class="w-full" multiple="multiple"
            data-placeholder="Search & pick products (leave empty to apply to all)">
            @php
            ...
            @foreach ($products as $product)
                <option value="{{ $product->id }}"
                    ...
                    {{ $product->name }}@if ($product->sku)
                        <small class="text-ink-tertiary">[{{ $product->sku }}]</small>
                    @endif
                </option>
            @endforeach
        </select>
        <small class="text-ink-tertiary">Each selected product shows as a removable tag. Leave empty to apply to all
            products.</small>
    </div>
    <div class="col-span-full">
    ...
    <script>
        ...
        $(function() {
            ...
            $select.select2({
                ...
                language: {
                    noResults: function() {
                        return 'No products found. Try another search term.';
                    }
                }
            });
        });
