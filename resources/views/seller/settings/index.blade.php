                ...
                <div class="min-w-0">
                    ...
                    <div class="flex flex-wrap items-center gap-2 mb-2">
                        ...
                        <span
                            ...
                            <i data-lucide="sliders-horizontal" style="width:11px;height:11px;" class="me-1"></i>
                            Configuration
                        </span>
                    </div>
                    <p class="text-sm text-ink-secondary mb-0">Manage shop details, address, identity documents and media in
                        one place.</p>
                </div>
            </div>
        </div>
    ...
    <section class="bg-white rounded-sm shadow-sm overflow-hidden">
        ...
        <div class="px-5 pt-3 flex flex-wrap gap-2 border-b border-border" role="tablist">
            @foreach ($tabs as $i => $tab)
                <button
                    class="flex items-center gap-2 px-3 py-2 text-sm font-semibold rounded-t-xs transition-colors {{ $i === 0 ? 'text-brand-deep border-b-2 border-brand bg-brand-tint' : 'text-ink-tertiary hover:text-ink-emphasis hover:bg-surface-muted' }}"
                    id="{{ $tab['key'] }}-tab" data-bs-toggle="tab" data-bs-target="#{{ $tab['key'] }}" type="button"
                    role="tab">
            ...
            <div class="tab-content p-6" id="settingsTabContent">
                ...
                <div class="tab-pane fade show active" id="info" role="tabpanel">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div>
                            <label
                                class="block text-[11px] font-semibold text-ink-tertiary mb-1 uppercase tracking-wider">Shop
                                Type </label>
                            <select name="shop_type" id="shop_type"
                                ...
                                @foreach (['individual' => 'Individual', 'business' => 'Business', 'company' => 'Company'] as $value => $label)
                                    <option value="{{ $value }}"
                                        {{ old('shop_type', $seller->shop_type) === $value ? 'selected' : '' }}>
                                        {{ $label }}</option>
                                @endforeach
                            </select>
                        </div>
                    ...
                    <div class="mt-4">
                        <label class="block text-[11px] font-semibold text-ink-tertiary mb-1 uppercase tracking-wider">Shop
                            Description</label>
                        ...
                        <small class="text-ink-tertiary mt-1 block">Tell customers about your shop (max 5000
                            characters).</small>
                    </div>
                ...
                <div class="tab-pane fade" id="address" role="tabpanel">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div>
                            <label
                                class="block text-[11px] font-semibold text-ink-tertiary mb-1 uppercase tracking-wider">Division</label>
                            <select name="division_id" id="divisionSelect"
                                class="w-full px-3 py-2 text-sm text-ink-emphasis bg-surface-muted rounded-xs focus:outline-none focus:ring-1 focus:ring-brand-deep transition-colors">
                                <option value="">Select Division</option>
                                @foreach ($divisions as $division)
                                    <option value="{{ $division->id }}"
                                        {{ (string) old('division_id', $seller->division_id) === (string) $division->id ? 'selected' : '' }}>
                                        {{ $division->name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div>
                            <label
                                class="block text-[11px] font-semibold text-ink-tertiary mb-1 uppercase tracking-wider">District</label>
                        ...
                        <div>
                            <label
                                class="block text-[11px] font-semibold text-ink-tertiary mb-1 uppercase tracking-wider">Default
                                Shipping Cost</label>
                            <input type="number" id="shipping_cost" name="shipping_cost"
                                value="{{ old('shipping_cost', $seller->shipping_cost) }}" step="0.01"
                                class="w-full px-3 py-2 text-sm text-ink-emphasis bg-surface-muted rounded-xs focus:outline-none focus:ring-1 focus:ring-brand-deep placeholder:text-ink-tertiary transition-colors">
                            <small class="text-ink-tertiary mt-1 block">Applied to your products unless overridden per
                                product.</small>
                        </div>
                    </div>
                    <div class="mt-6 flex justify-end pt-4 border-t border-border">
                ...
                <div class="tab-pane fade" id="identity" role="tabpanel">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            <label
                                class="block text-[11px] font-semibold text-ink-tertiary mb-1 uppercase tracking-wider">NID
                                Number</label>
                            <input type="text" name="nid_no" value="{{ old('nid_no', $seller->nid_no) }}"
                                class="w-full px-3 py-2 text-sm text-ink-emphasis bg-surface-muted rounded-xs focus:outline-none focus:ring-1 focus:ring-brand-deep placeholder:text-ink-tertiary transition-colors">
                        </div>
                        <div>
                            <label
                                class="block text-[11px] font-semibold text-ink-tertiary mb-1 uppercase tracking-wider">Trade
                                License No</label>
                ...
                <div class="tab-pane fade" id="media" role="tabpanel">
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                        <div>
                            ...
                            <small class="text-ink-tertiary mt-1 block">Used across the marketplace storefront and admin
                                listings.</small>
                        </div>
                        <div>
                            <label
                                class="block text-[11px] font-semibold text-ink-tertiary mb-1 uppercase tracking-wider">Cover
                                Photo</label>
            ...
            (function() {
                ...
                $form.on('submit', function(e) {
                    ...
                    $submitBtn.prop('disabled', true).html(
                        '<i data-lucide="loader-circle" class="animate-spin" style="width:16px;height:16px;"></i> Saving...'
                        );
                        ...
                        error: function(xhr) {
                            ...
                            if (xhr.status === 422 && xhr.responseJSON && xhr.responseJSON.errors) {
                                ...
                                $.each(errors, function(field, arr) {
                                    const label = field.replace(/_/g, ' ').replace(/\b\w/g, c =>
                                        c.toUpperCase());
