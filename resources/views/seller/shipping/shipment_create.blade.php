...
@section('content')
    <div class="w-full px-0">
        <div class="flex items-center gap-2 mb-3">
            ...
            <span
                class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-semibold badge-soft-primary">#{{ $order->invoice_id }}</span>
        </div>
                    ...
                    <form method="POST" action="{{ route('seller.shipping.shipments.store') }}">
                        ...
                        <div class="p-5">
                            <div class="grid grid-cols-1 md:grid-cols-12 gap-3">
                                ...
                                <div class="md:col-span-6">
                                    <label class="block text-xs font-medium text-ink-secondary mb-1">Tracking Number
                                    </label>
                                    <input type="text" name="tracking_number"
                                        class="w-full px-3 py-2 text-sm text-ink bg-white border border-border rounded-xs focus:outline-none focus:border-brand-deep focus:ring-1 focus:ring-brand-deep placeholder:text-ink-tertiary transition-colors"
                                        placeholder="e.g., STF123456789" required>
                                </div>
                                ...
                                <div class="md:col-span-4">
                                    <label class="block text-xs font-medium text-ink-secondary mb-1">Shipping Cost
                                        (৳)</label>
                                ...
                                <div class="md:col-span-4">
                                    ...
                                    <input type="number" step="0.01" min="0" name="cod_amount"
                                        class="w-full px-3 py-2 text-sm text-ink bg-white border border-border rounded-xs focus:outline-none focus:border-brand-deep focus:ring-1 focus:ring-brand-deep placeholder:text-ink-tertiary transition-colors">
                                </div>
