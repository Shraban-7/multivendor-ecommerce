    ...
    <div class="p-5 lg:p-6 pt-6">
        <div class="flex flex-wrap items-start justify-between gap-4">
            <div class="min-w-0">
                <nav class="flex items-center gap-1 mb-2 text-xs text-ink-tertiary">
                    <i data-lucide="shopping-cart" class="text-brand-deep" style="width:12px;height:12px;"></i>
                    <span>Reports</span>
                    <i data-lucide="chevron-right" style="width:12px;height:12px;"></i>
                    <span class="text-ink-soft font-semibold">Sales Report</span>
                </nav>
                <div class="flex flex-wrap items-center gap-2 mb-2">
                    <h1 class="text-xl font-bold text-ink-emphasis mb-0">Sales Report</h1>
                    <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-bold uppercase tracking-wider bg-brand-tint text-brand-deep">
                        <i data-lucide="shopping-cart" style="width:11px;height:11px;" class="me-1"></i> {{ $rangeText }}
                    </span>
                </div>
                <p class="text-sm text-ink-secondary mb-0">Detailed channel mix, category performance and top products.</p>
            </div>
        ...
        <div class="mt-4 flex flex-wrap gap-2 text-sm items-center text-ink-secondary">
            ...
            @foreach ($rangeLabels as $key => $label)
                ...
                @else
                    <a href="{{ route('seller.reports.sales', ['range' => $key]) }}" class="px-2 py-0.5 rounded-xs transition-colors {{ $rangeValue == $key ? 'bg-brand-tint text-brand-deep font-semibold' : 'hover:bg-surface-muted text-ink-secondary' }}">{{ $label }}</a>
                @endif
            @endforeach
        </div>
    </div>
