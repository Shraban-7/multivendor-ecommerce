...
<section class="grid grid-cols-1 md:grid-cols-3 gap-3">
    <div class="md:col-span-2">
        <form id="profileForm" action="{{ route('seller.profile', $seller->username) }}" method="POST" enctype="multipart/form-data">
            @csrf
                ...
                <div class="p-5 border-t border-border">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
                        ...
                        <div>
                            ...
                            <input type="text" id="phone" name="phone" value="{{ old('phone', $seller->phone) }}" required
                                   class="w-full px-3 py-2 text-sm text-ink-emphasis bg-surface-muted rounded-xs focus:outline-none focus:ring-1 focus:ring-brand-deep placeholder:text-ink-tertiary transition-colors">
                        </div>
                    </div>
                </div>
                ...
                <div class="p-5 space-y-3">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
                        <div>
                            ...
                            <input type="text" id="business_name" name="business_name" value="{{ old('business_name', $seller->business_name) }}" required
                                   class="w-full px-3 py-2 text-sm text-ink-emphasis bg-surface-muted rounded-xs focus:outline-none focus:ring-1 focus:ring-brand-deep placeholder:text-ink-tertiary transition-colors">
                        </div>
                        <div>
                            ...
                            <input type="email" id="business_email" name="business_email" value="{{ old('business_email', $seller->business_email) }}" required
                                   class="w-full px-3 py-2 text-sm text-ink-emphasis bg-surface-muted rounded-xs focus:outline-none focus:ring-1 focus:ring-brand-deep placeholder:text-ink-tertiary transition-colors">
                        </div>
                    </div>
                    <div>
            ...
            <div class="p-5 border-t border-border flex flex-col gap-3">
                <div class="flex items-center gap-3">
                    <img src="{{ storage_url($seller->image) }}" alt="{{ $seller->name }}"
                         style="width:64px;height:64px;object-fit:cover;border-radius:12px;" class="shrink-0 bg-surface-muted">
                    <div>
                        <div class="font-bold text-ink-emphasis">{{ $seller->name }}</div>
                        <small class="text-ink-tertiary">@<span class="font-semibold">{{ $seller->username }}</span></small>
                    </div>
                </div>
                <hr class="border-border">
                <div class="text-sm space-y-1.5">
                    <div class="flex justify-between"><span class="text-ink-tertiary">Email</span><span class="text-ink-emphasis font-medium">{{ $seller->email ?? '—' }}</span></div>
                    <div class="flex justify-between"><span class="text-ink-tertiary">Phone</span><span class="text-ink-emphasis font-medium">{{ $seller->phone ?? '—' }}</span></div>
                    <div class="flex justify-between"><span class="text-ink-tertiary">Joined</span><span class="text-ink-emphasis font-medium">{{ $seller->created_at->format('d M Y') }}</span></div>
                </div>
                <hr class="border-border">
