                        ...
                        <div>
                            ...
                            <select name="role_id" required
                                ...
                                @foreach ($roles as $role)
                                    <option value="{{ $role->id }}" @selected($admin->role_id == $role->id)>{{ $role->title }}
                                    </option>
                                @endforeach
                            </select>
                            <small class="text-ink-tertiary mt-1 block">Move the admin to a different permission set without
                                resetting their password.</small>
                        </div>
                        <div>
                            <label class="block text-xs font-semibold text-ink-secondary mb-1 uppercase tracking-wider">Reset
                                password</label>
                            ...
                            <small class="text-ink-tertiary mt-1 block">Only fill in to set a new password. The current password
                                remains otherwise.</small>
                        </div>
                    @else
                        <div class="p-3 rounded-xs bg-amber-50 flex items-start gap-2 text-xs text-ink-secondary">
                            <i data-lucide="lock" class="text-feedback-warning shrink-0 mt-0.5"
            ...
            <section class="bg-white rounded-sm shadow-sm overflow-hidden">
                ...
                <div class="p-5">
                    <div class="flex items-center gap-3 mb-4">
                        <span
                            class="shrink-0 w-14 h-14 rounded-full {{ $isSuper ? 'bg-rose-50 text-rose-500' : 'bg-info-tint text-feedback-info' }} flex items-center justify-center font-bold text-lg">
                            @if ($isSuper)
                    ...
                    <dl class="text-sm space-y-2">
                        <div class="flex justify-between">
                            ...
                            <dd>
                                <span
                                    class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-semibold {{ $isSuper ? 'bg-rose-500 text-white' : 'bg-info-tint text-feedback-info' }}">
