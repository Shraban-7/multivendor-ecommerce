...
@section('content')
    <div class="flex justify-between items-start mb-4">
        <div>
            ...
            <p class="text-sm text-ink-secondary mt-1 font-mono">{{ $coupon->code }}@if($coupon->title) · {{ $coupon->title }}@endif</p>
        </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-4">
                    ...
                    <div class="p-5 space-y-4">
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                ...
                                <input type="text" name="title" class="w-full px-3 py-2 text-sm text-ink bg-white border border-border rounded-xs focus:outline-none focus:border-brand-deep focus:ring-1 focus:ring-brand-deep placeholder:text-ink-tertiary transition-colors" value="{{ old('title', $coupon->title) }}" placeholder="e.g., Summer Sale 2026">
                            </div>
                            <div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                ...
                                <input type="number" step="0.01" min="0" name="max_discount" class="w-full px-3 py-2 text-sm text-ink bg-white border border-border rounded-xs focus:outline-none focus:border-brand-deep focus:ring-1 focus:ring-brand-deep placeholder:text-ink-tertiary transition-colors" value="{{ old('max_discount', $coupon->max_discount) }}" placeholder="No cap">
                            </div>
                            <div>
                                <label class="block text-xs font-medium text-ink-secondary mb-1">Min Purchase</label>
                                <input type="number" step="0.01" min="0" name="min_purchase" class="w-full px-3 py-2 text-sm text-ink bg-white border border-border rounded-xs focus:outline-none focus:border-brand-deep focus:ring-1 focus:ring-brand-deep placeholder:text-ink-tertiary transition-colors" value="{{ old('min_purchase', $coupon->min_purchase) }}" placeholder="No minimum">
                            </div>
                        </div>

                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                <label class="block text-xs font-medium text-ink-secondary mb-1">Usage Limit <span class="text-ink-tertiary text-xs">(blank = unlimited)</span></label>
                                <input type="number" min="1" name="usage_limit" class="w-full px-3 py-2 text-sm text-ink bg-white border border-border rounded-xs focus:outline-none focus:border-brand-deep focus:ring-1 focus:ring-brand-deep placeholder:text-ink-tertiary transition-colors" value="{{ old('usage_limit', $coupon->usage_limit) }}" placeholder="Unlimited">
                            </div>
                            <div>
                                ...
                                <select name="status" class="w-full px-3 py-2 text-sm text-ink bg-white border border-border rounded-xs focus:outline-none focus:border-brand-deep focus:ring-1 focus:ring-brand-deep transition-colors">
                                    <option value="1" {{ old('status', (string) $coupon->status) == '1' ? 'selected' : '' }}>Active</option>
                                    <option value="0" {{ old('status', (string) $coupon->status) === '0' ? 'selected' : '' }}>Inactive</option>
                                </select>
                            </div>
                        </div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                ...
                                <input type="date" name="valid_from" class="w-full px-3 py-2 text-sm text-ink bg-white border border-border rounded-xs focus:outline-none focus:border-brand-deep focus:ring-1 focus:ring-brand-deep transition-colors" value="{{ old('valid_from', $coupon->valid_from->format('Y-m-d')) }}" required>
                            </div>
                            <div>
                                <label class="block text-xs font-medium text-ink-secondary mb-1">Valid Until <span class="text-feedback-danger">*</span></label>
                                <input type="date" name="valid_until" class="w-full px-3 py-2 text-sm text-ink bg-white border border-border rounded-xs focus:outline-none focus:border-brand-deep focus:ring-1 focus:ring-brand-deep transition-colors" value="{{ old('valid_until', $coupon->valid_until->format('Y-m-d')) }}" required>
                            </div>
                        </div>
                        ...
                        @if ($coupon->seller)
                            <div class="p-3 bg-brand-tint border border-border rounded-xs flex items-center gap-3">
                                ...
                                <div>
                                    <div class="text-xs text-ink-tertiary uppercase tracking-wider font-semibold">Seller Coupon</div>
                                    <div class="font-semibold text-ink">{{ $coupon->seller->business_name ?? $coupon->seller->name }}</div>
                                </div>
                            </div>
                        @endif
        ...
        <div class="lg:col-span-1">
            <div class="bg-white border border-border rounded-sm shadow-sm overflow-hidden">
                <div class="bg-surface-muted px-4 py-2.5 border-b border-border">
                    <h6 class="text-xs font-semibold text-ink uppercase tracking-wider"><i data-lucide="activity" class="icon-xs me-1"></i> Snapshot</h6>
                </div>
                <div class="p-5 space-y-3 text-sm">
                    ...
                    @endif
                    <div class="flex items-center justify-between"><span class="text-ink-secondary">Valid From</span><span class="text-ink">{{ $coupon->valid_from->format('d M Y') }}</span></div>
                    <div class="flex items-center justify-between"><span class="text-ink-secondary">Valid Until</span><span class="text-ink">{{ $coupon->valid_until->format('d M Y') }}</span></div>
                    @if($coupon->discount_type === 'percentage')
                        <div class="flex items-center justify-between"><span class="text-ink-secondary">Discount</span><span class="text-ink font-semibold">{{ $coupon->discount_value }}% @if($coupon->max_discount) (cap {{ money($coupon->max_discount) }})@endif</span></div>
                    @else
                        <div class="flex items-center justify-between"><span class="text-ink-secondary">Discount</span><span class="text-ink font-semibold">{{ money($coupon->discount_value) }}</span></div>
                    @endif
                    @if($coupon->min_purchase)
                        <div class="flex items-center justify-between"><span class="text-ink-secondary">Min Purchase</span><span class="text-ink">{{ money($coupon->min_purchase) }}</span></div>
                    @endif
                </div>
            </div>
        </div>
