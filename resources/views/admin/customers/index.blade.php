        ...
        <div class="flex flex-wrap items-start justify-between gap-4">
            <div class="min-w-0">
                ...
                <div class="flex flex-wrap items-center gap-2 mb-2">
                    <h1 class="text-xl font-bold text-ink-emphasis mb-0">Customers</h1>
                    <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-bold uppercase tracking-wider bg-feedback-info/15 text-feedback-info">
                        <i data-lucide="globe" style="width:11px;height:11px;" class="me-1"></i> Website Users
                    </span>
                </div>
                <p class="text-sm text-ink-secondary mb-0">All customers who registered and purchased through the website.</p>
            </div>
        </div>
    </div>
</section>
...
@php
    $countCards = [
        ...
        ['key' => 'with_orders', 'label' => 'Ordered',         'top' => '#0ea5e9', 'text' => 'text-feedback-info',     'icon' => 'shopping-bag'],
    ];
@endphp
...
<section class="bg-white rounded-sm shadow-sm overflow-hidden">
    <div class="px-5 py-3 bg-surface-muted flex items-center gap-2">
        ...
        @if ($filters['search'] || ($filters['verified'] ?? null))
            <a href="{{ route('admin.customers.index') }}" class="text-[11px] font-semibold text-ink-tertiary hover:text-ink-emphasis inline-flex items-center gap-1">
                <i data-lucide="x" style="width:11px;height:11px;"></i> Clear
            </a>
        @endif
    </div>
    <div class="p-4 border-t border-border">
        <form method="GET" action="{{ route('admin.customers.index') }}" class="grid grid-cols-1 md:grid-cols-12 gap-2">
                    ...
                    <tr class="hover:bg-surface-muted/40 transition-colors">
                        <td class="px-4 py-3">
                            <div class="flex items-center gap-3">
                                ...
                                <div class="min-w-0">
                                    <a href="{{ route('admin.customers.profile', $customer->username) }}"
                                       class="font-semibold text-ink-emphasis text-sm hover:text-brand-deep truncate max-w-[180px] inline-block">
                                        {{ $customer->name }}
                                    </a>
                                    <div class="text-xs text-ink-tertiary">
                        ...
                        <td class="px-4 py-3 text-center">
                            <span class="inline-flex items-center justify-center px-2.5 py-0.5 rounded-full text-[11px] font-bold {{ ($customer->orders_count ?? 0) > 0 ? 'bg-feedback-info/15 text-feedback-info' : 'bg-surface-muted text-ink-emphasis' }}">
                        ...
                        <td class="px-4 py-3 text-right">
                            <a href="{{ route('admin.customers.profile', $customer->username) }}" class="btn btn-light btn-sm">
                    ...
                    <tr>
                        <td colspan="8">
                            <div class="py-10 text-center">
                                ...
                                <p class="text-ink-soft font-semibold mb-1">No customers yet</p>
                                <p class="text-ink-tertiary text-xs">Registered website customers will appear here as soon as they sign up.</p>
                            </div>
                        </td>
                    </tr>
                @endforelse
