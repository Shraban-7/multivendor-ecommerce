        ...
        <div class="p-5 lg:p-6 pt-6">
            <div class="flex flex-wrap items-start justify-between gap-4">
                <div class="min-w-0">
                    ...
                    <p class="text-sm text-ink-secondary mb-0">Manage seller subscription plans, statuses, billing cycles
                        and renewals in one place.</p>
                </div>
                <div>
    ...
    <section class="grid grid-cols-2 md:grid-cols-4 gap-3 mb-4">
        <article class="bg-white rounded-sm shadow-sm p-4 relative overflow-hidden">
            ...
            <div class="flex items-start justify-between gap-3 mt-1">
                ...
                <span
                    class="shrink-0 w-10 h-10 rounded-sm bg-emerald-50 text-feedback-success flex items-center justify-center">
        ...
        <article class="bg-white rounded-sm shadow-sm p-4 relative overflow-hidden">
            ...
            <div class="flex items-start justify-between gap-3 mt-1">
                ...
                <span
                    class="shrink-0 w-10 h-10 rounded-sm bg-warning-tint text-feedback-warning flex items-center justify-center">
        ...
        <article class="bg-white rounded-sm shadow-sm p-4 relative overflow-hidden">
            ...
            <div class="flex items-start justify-between gap-3 mt-1">
                ...
                <span
                    class="shrink-0 w-10 h-10 rounded-sm bg-info-tint text-feedback-info flex items-center justify-center">
        ...
        <form method="GET" action="{{ route('admin.subscriptions.index') }}" class="p-5">
            <div class="grid grid-cols-1 md:grid-cols-12 gap-3">
                <div class="md:col-span-5">
                    <label
                        class="block text-[11px] font-semibold text-ink-tertiary mb-1 uppercase tracking-wider">Search</label>
                ...
                <div class="md:col-span-2">
                    <label
                        class="block text-[11px] font-semibold text-ink-tertiary mb-1 uppercase tracking-wider">Status</label>
                    <select name="status"
                ...
                <div class="md:col-span-3">
                    <label
                        class="block text-[11px] font-semibold text-ink-tertiary mb-1 uppercase tracking-wider">Plan</label>
                    <select name="plan_id"
                        class="w-full px-3 py-2 text-sm text-ink-emphasis bg-surface-muted rounded-xs focus:outline-none focus:ring-1 focus:ring-brand-deep transition-colors">
            ...
            @if (request()->filled('search') || request()->filled('status') || request()->filled('plan_id'))
                <div class="mt-3 flex items-center gap-2 text-xs">
                    ...
                    @if (request()->filled('search'))
                        <span
                            class="inline-flex items-center px-2 py-0.5 rounded-full bg-brand-tint text-brand-deep">Search:
                            {{ request('search') }}</span>
                    @endif
                    @if (request()->filled('status'))
                        <span
                            class="inline-flex items-center px-2 py-0.5 rounded-full bg-brand-tint text-brand-deep">Status:
                            {{ $statusMap[request('status')]['label'] ?? request('status') }}</span>
                    @endif
                    @if (request()->filled('plan_id') && $plans->where('id', request('plan_id'))->first())
                        <span class="inline-flex items-center px-2 py-0.5 rounded-full bg-brand-tint text-brand-deep">Plan:
                            {{ $plans->where('id', request('plan_id'))->first()->name }}</span>
                    @endif
    ...
    <section class="bg-white rounded-sm shadow-sm overflow-hidden">
        <div class="px-5 py-3 flex items-center justify-between">
            <div class="flex items-center gap-2">
                ...
                <span class="text-ink-tertiary text-xs">· {{ $subscriptions->total() }}
                    {{ Str::plural('record', $subscriptions->total()) }}</span>
            </div>
        </div>
            ...
            <table class="w-full text-left text-sm text-ink-soft">
                ...
                <tbody class="divide-y divide-border">
                    @forelse ($subscriptions as $subscription)
                        @php
                            $meta = $statusMap[$subscription->status] ?? [
                                'label' => ucfirst($subscription->status),
                                'pill' => 'bg-surface-muted text-ink-soft',
                                'icon' => 'circle',
                            ];
                        ...
                        <tr class="hover:bg-surface-muted/50 transition-colors">
                            ...
                            <td class="px-4 py-3">
                                <div class="flex items-center gap-2">
                                    <img src="{{ $subscription->seller?->businessAvatar }}" alt=""
                                        width="32" height="32"
                                        class="rounded-sm object-cover border border-border shrink-0"
                                        style="width:32px;height:32px;">
                                    <div class="min-w-0">
                                        <p class="mb-0 font-medium text-ink-emphasis truncate">
                                            {{ $subscription->seller->name ?? 'N/A' }}</p>
                            ...
                            <td class="px-4 py-3">
                                <div class="flex items-center gap-2 flex-wrap">
                                    <span
                                        class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium bg-info-tint text-feedback-info">
                                    ...
                                    @if ($subscription->is_trial)
                                        <span
                                            ...
                                            <i data-lucide="gift" style="width:11px;height:11px;" class="me-1"></i>
                                            Trial
                                        </span>
                                    @endif
                                </div>
                            ...
                            <td class="px-4 py-3 text-center">
                                <span
                                    class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium text-white {{ $meta['pill'] }}">
                                    <i data-lucide="{{ $meta['icon'] }}" style="width:11px;height:11px;"
                                        class="me-1"></i>
                                    {{ $meta['label'] }}
                                </span>
                            </td>
                            <td class="px-4 py-3 text-ink-secondary">
                                {{ $subscription->start_date?->format('d M, Y') ?? '—' }}</td>
                            <td class="px-4 py-3 text-ink-secondary">
                                {{ $subscription->end_date?->format('d M, Y') ?? '—' }}</td>
                            <td class="px-4 py-3 text-center">
                                @if ($daysLeft > 0)
                                    <span
                                        class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-semibold text-white {{ $daysLeft <= 3 ? 'bg-rose-500' : 'bg-emerald-500' }}">
                                        <i data-lucide="clock" style="width:11px;height:11px;" class="me-1"></i>
                                        {{ $daysLeft }}d
                                    </span>
                                @else
                                    <span
                                        class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium bg-surface-muted text-ink-tertiary">
                                        Expired
                                    </span>
                                @endif
                            </td>
                            <td class="px-4 py-3 text-right">
                                <div class="flex justify-end gap-1">
                                    ...
                                    @if (in_array($subscription->status, ['active', 'trial'], true))
                                        <button type="button" class="btn btn-warning btn-sm" title="Suspend"
                                            data-bs-toggle="modal" data-bs-target="#suspendModal{{ $subscription->id }}">
                    ...
                    @empty
                        <tr>
                            <td colspan="8" class="px-4 py-10 text-center text-sm text-ink-tertiary">
                                <i data-lucide="inbox" class="mx-auto mb-3 opacity-50"
        ...
        @if ($subscriptions->hasPages())
            <div class="px-5 py-3 bg-surface-muted flex items-center justify-between">
                <small class="text-ink-tertiary">Showing
                    {{ $subscriptions->firstItem() }}–{{ $subscriptions->lastItem() }} of
                    {{ $subscriptions->total() }}</small>
                {{ $subscriptions->links() }}
            </div>
        @endif
    </section>
                    ...
                    <div class="modal-content border-0 shadow-lg">
                        <form action="{{ route('admin.subscriptions.suspend', $subscription) }}" method="POST">
                            ...
                            <div class="modal-header border-b border-border bg-surface-muted">
                                <div class="flex items-center gap-2">
                                    <span
                                        class="shrink-0 w-9 h-9 rounded-sm bg-warning-tint text-feedback-warning flex items-center justify-center">
                            ...
                            <div class="modal-body">
                                ...
                                <label
                                    class="block text-xs font-semibold text-ink-secondary mb-1 uppercase tracking-wider">Reason
