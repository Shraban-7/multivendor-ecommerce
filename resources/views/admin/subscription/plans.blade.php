    ...
    <div class="p-5 lg:p-6 pt-6">
        <div class="flex flex-wrap items-start justify-between gap-4">
            <div class="min-w-0">
                ...
                <p class="text-sm text-ink-secondary mb-0">Define the pricing tiers, product limits and feature flags you sell to sellers.</p>
            </div>
            <div>
                <a href="{{ route('admin.subscriptions.index') }}" class="btn btn-light btn-sm me-1">
                    <i data-lucide="users" class="icon-xs"></i> Subscribers
                </a>
                <button type="button" class="btn btn-primary btn-sm" data-bs-toggle="modal" data-bs-target="#planModal" id="addPlanBtn">
            ...
            <article class="bg-white rounded-sm shadow-sm overflow-hidden h-full relative">
                ...
                <div class="p-5 flex flex-col h-full">
                    <div class="flex items-start justify-between gap-2 mb-3">
                        <div class="min-w-0">
                            <p class="text-[11px] font-bold uppercase tracking-wider text-ink-tertiary">Tier {{ $i + 1 }}</p>
                            <h3 class="font-bold text-xl text-ink-emphasis mb-0 truncate">{{ $plan->name }}</h3>
                        </div>
                        <div class="flex items-center gap-1">
                            <button type="button"
                                    class="btn btn-light btn-sm editPlanBtn"
                                    data-plan='@json($plan)'
                                    title="Edit plan">
                                <i data-lucide="edit" class="icon-xs"></i>
                            </button>
                            <button type="button"
                                    class="btn btn-danger btn-sm deletePlanBtn"
                                    data-id="{{ $plan->id }}"
                                    data-name="{{ $plan->name }}"
                                    title="Delete plan">
                    ...
                    <div class="text-center mb-3">
                        <div class="flex items-baseline justify-center gap-1">
                            <h2 class="font-bold text-ink-emphasis" style="font-size:2.4rem;line-height:1;">{{ money($plan->price) }}</h2>
                        </div>
                        <small class="text-ink-tertiary">per {{ $plan->duration_type === 'yearly' ? 'year' : 'month' }}</small>
                    </div>

                    <ul class="space-y-2 text-sm mb-4 grow">
                        <li class="flex items-start gap-2">
                            <i data-lucide="check-circle" class="text-feedback-success shrink-0 mt-0.5" style="width:16px;height:16px;"></i>
                            <span class="text-ink-emphasis"><strong>{{ $plan->commission_rate }}%</strong> commission</span>
                        </li>
                        <li class="flex items-start gap-2">
                            <i data-lucide="check-circle" class="text-feedback-success shrink-0 mt-0.5" style="width:16px;height:16px;"></i>
                        ...
                        @foreach (['pos_access', 'analytics', 'priority_support', 'custom_domain', 'payment_checker'] as $feature)
                            @if (!empty($plan->{$feature}))
                                <li class="flex items-start gap-2">
                                    <i data-lucide="check-circle" class="text-feedback-success shrink-0 mt-0.5" style="width:16px;height:16px;"></i>
                                    <span class="text-ink-emphasis">{{ $featureLabels[$feature]['label'] ?? ucwords(str_replace('_', ' ', $feature)) }}</span>
                                </li>
                            @endif
                        @endforeach
                        @foreach (['pos_access', 'analytics', 'priority_support', 'custom_domain', 'payment_checker'] as $feature)
                            @if (empty($plan->{$feature}))
                                <li class="flex items-start gap-2">
                                    <i data-lucide="minus-circle" class="text-ink-tertiary shrink-0 mt-0.5" style="width:16px;height:16px;"></i>
                                    <span class="text-ink-tertiary line-through">{{ $featureLabels[$feature]['label'] ?? ucwords(str_replace('_', ' ', $feature)) }}</span>
                                </li>
                            @endif
                        @endforeach
                    </ul>
                    ...
                    <div class="flex gap-2 pt-3 border-t border-border">
                        <button type="button" class="btn btn-primary btn-sm flex-1 editPlanBtn" data-plan='@json($plan)'>
            ...
            <div class="modal-content border-0 shadow-lg">
                ...
                <div class="modal-body p-5">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            <label class="block text-xs font-semibold text-ink-secondary mb-1 uppercase tracking-wider">Plan Name <span class="text-feedback-danger">*</span></label>
                        ...
                        <div>
                            <label class="block text-xs font-semibold text-ink-secondary mb-1 uppercase tracking-wider">Price (৳) <span class="text-feedback-danger">*</span></label>
                            <input type="number" id="price" min="0" step="0.01" required
                                   class="w-full px-3 py-2 text-sm text-ink-emphasis bg-surface-muted rounded-xs focus:outline-none focus:ring-1 focus:ring-brand-deep placeholder:text-ink-tertiary transition-colors">
                        </div>
                        <div>
                            <label class="block text-xs font-semibold text-ink-secondary mb-1 uppercase tracking-wider">Duration</label>
                            <select id="duration_type" class="w-full px-3 py-2 text-sm text-ink-emphasis bg-surface-muted rounded-xs focus:outline-none focus:ring-1 focus:ring-brand-deep transition-colors">
                                <option value="monthly">Monthly</option>
                                <option value="yearly">Yearly</option>
                            </select>
                        </div>
                        <div>
                            <label class="block text-xs font-semibold text-ink-secondary mb-1 uppercase tracking-wider">Commission Rate (%) <span class="text-feedback-danger">*</span></label>
                            <input type="number" id="commission_rate" step="0.01" required
                                   class="w-full px-3 py-2 text-sm text-ink-emphasis bg-surface-muted rounded-xs focus:outline-none focus:ring-1 focus:ring-brand-deep placeholder:text-ink-tertiary transition-colors">
                        </div>
                        <div>
                            <label class="block text-xs font-semibold text-ink-secondary mb-1 uppercase tracking-wider">Product Limit</label>
                            <input type="number" id="product_limit" min="0" required
                                   class="w-full px-3 py-2 text-sm text-ink-emphasis bg-surface-muted rounded-xs focus:outline-none focus:ring-1 focus:ring-brand-deep placeholder:text-ink-tertiary transition-colors">
                            <small class="text-ink-tertiary mt-1 block">0 = unlimited</small>
                        </div>
                        <div>
                            <label class="block text-xs font-semibold text-ink-secondary mb-1 uppercase tracking-wider">Staff Accounts</label>
                    ...
                    <div class="mt-4">
                        <p class="block text-xs font-semibold text-ink-secondary mb-2 uppercase tracking-wider">Feature flags</p>
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-2">
                            @foreach (['pos_access' => 'POS Access', 'analytics' => 'Analytics', 'priority_support' => 'Priority Support', 'custom_domain' => 'Custom Domain', 'payment_checker' => 'Payment Checker'] as $key => $label)
                                <label class="flex items-center gap-2 px-3 py-2 rounded-xs bg-surface-muted cursor-pointer hover:bg-brand-tint transition-colors">
                                    <input type="checkbox" id="{{ $key }}" class="h-4 w-4 rounded border-border text-brand focus:ring-brand focus:ring-2">
...
$(document).ready(function () {
    ...
    $('.editPlanBtn').on('click', function () {
        ...
        ['pos_access', 'analytics', 'priority_support', 'custom_domain', 'payment_checker'].forEach(function (key) {
