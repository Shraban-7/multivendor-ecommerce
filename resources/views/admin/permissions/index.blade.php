@php
    ...
    $roleTone = [
        ...
        'manager' => [
            'bar' => 'bg-amber-500',
            'pill' => 'bg-warning-tint text-feedback-warning',
            'icon' => 'briefcase',
        ],
        ...
        <div class="p-5 lg:p-6 pt-6">
            <div class="flex flex-wrap items-start justify-between gap-4">
                <div class="min-w-0">
                    ...
                    <p class="text-sm text-ink-secondary mb-0">Define roles, group permissions, and assign them to admins.
                    </p>
                </div>
                @if ($isSuperAdmin)
                    <div>
        ...
        <article class="bg-white rounded-sm shadow-sm p-4 relative overflow-hidden">
            ...
            <div class="flex items-start justify-between gap-3 mt-1">
                <div class="min-w-0 flex-1">
                    ...
                    <h3 class="mb-0 font-bold text-2xl text-ink-emphasis mt-1">{{ number_format($permissions->count()) }}
                    </h3>
            ...
            <div class="flex items-start justify-between gap-3 mt-1">
                <div class="min-w-0 flex-1">
                    ...
                    <h3 class="mb-0 font-bold text-2xl text-ink-emphasis mt-1">
                        {{ number_format($roles->where('name', 'super_admin')->count()) }}</h3>
    ...
    <section class="bg-white rounded-sm shadow-sm overflow-hidden">
        <div class="px-5 py-3 flex items-center justify-between">
            <div class="flex items-center gap-2">
                ...
                <span class="text-ink-tertiary text-xs">· {{ $roles->count() }}
                    {{ Str::plural('role', $roles->count()) }}</span>
            </div>
        </div>
                        ...
                        <tr class="hover:bg-surface-muted/50 transition-colors">
                            <td class="px-4 py-3">
                                <div class="flex items-center gap-3">
                                    <span
                                        class="shrink-0 w-9 h-9 rounded-sm flex items-center justify-center {{ $tone['pill'] }}">
                            ...
                            <td class="px-4 py-3">
                                <div class="flex flex-wrap gap-1">
                                    @forelse ($role->permissions->take(8) as $permission)
                                        <span
                                            class="inline-flex items-center px-2 py-0.5 rounded-full text-[11px] font-semibold bg-brand-tint text-brand-deep">
                                            {{ $permission->title }}
                                        </span>
                                    @empty
                                    ...
                                    @if ($role->permissions->count() > 8)
                                        <span
                                            class="inline-flex items-center px-2 py-0.5 rounded-full text-[11px] font-semibold bg-surface-muted text-ink-emphasis">
                            ...
                            @if ($isSuperAdmin)
                                <td class="px-4 py-3 text-right">
                                    ...
                                    @else
                                        <span
                                            class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-semibold bg-rose-500 text-white">
                    ...
                    @empty
                        <tr>
                            <td colspan="{{ $isSuperAdmin ? 4 : 3 }}"
                                class="px-4 py-10 text-center text-sm text-ink-tertiary">
                                <i data-lucide="shield-off" class="mx-auto mb-3 opacity-50"
                    ...
                    <div class="modal-content border-0 shadow-lg">
                        <form action="{{ route('admin.roles.store') }}" method="POST">
                            ...
                            <div class="modal-body">
                                <label
                                    class="block text-[11px] font-semibold text-ink-tertiary mb-1 uppercase tracking-wider">Role
                                    Title </label>
                                ...
                                <small class="text-ink-tertiary mt-1 block">A short, human-friendly label. The internal slug is
