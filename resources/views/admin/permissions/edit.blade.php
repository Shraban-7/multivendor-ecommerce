            ...
            <div class="flex flex-wrap items-start justify-between gap-4">
                <div class="min-w-0">
                    <nav class="flex items-center gap-1 mb-2 text-xs text-ink-tertiary">
                        ...
                        <a href="{{ route('admin.roles.index') }}" class="hover:text-ink-soft transition-colors">Roles &
                            Permissions</a>
                    ...
                    <div class="flex flex-wrap items-center gap-2 mb-2">
                        ...
                        @endif
                        <span
                            class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-bold uppercase tracking-wider bg-info-tint text-feedback-info">
                            {{ count($permissionNames) }} of {{ $permissions->count() }} permissions
                        </span>
                    </div>
                    <p class="text-sm text-ink-secondary mb-0">{{ $role->name }} · created
            ...
            <section class="bg-white rounded-sm shadow-sm overflow-hidden">
                ...
                @if (hasPermission('admin.roles.update'))
                    <form id="editRoleForm" action="{{ route('admin.roles.update', $role->id) }}" method="POST"
                        ...
                        @csrf
                        <div>
                            <label class="block text-xs font-semibold text-ink-secondary mb-1 uppercase tracking-wider">Role
                                Title </label>
                            ...
                            @else
                                <small class="text-ink-tertiary mt-1 block">The internal slug <code
                                        class="font-mono">{{ $role->name }}</code> stays the same.</small>
                            @endif
                        </div>
                        ...
                        <div>
                            <div class="flex items-center justify-between mb-2 flex-wrap gap-2">
                                <div>
                                    <label
                                        class="block text-xs font-semibold text-ink-secondary uppercase tracking-wider">Permissions
                                        ({{ count($permissionNames) }} / {{ $permissions->count() }} active)</label>
                                    <small class="text-ink-tertiary block mt-0.5">Tick the capabilities this role
                                        unlocks.</small>
                                </div>
                                @unless ($isSuper)
                                    <label
                                        class="inline-flex items-center gap-2 px-3 py-1.5 text-xs font-semibold text-ink-emphasis bg-surface-muted rounded-xs cursor-pointer hover:bg-brand-tint transition-colors">
                            ...
                            <div class="rounded-xs bg-surface-muted p-4 space-y-4 max-h-[640px] overflow-y-auto">
                                @forelse ($permissionGroups as $group => $perms)
                                    <div>
                                        <div class="flex items-center gap-2 mb-2">
                                            <span
                                                class="text-[11px] font-bold tracking-wider text-ink-tertiary uppercase">{{ $group }}</span>
                                            <span
                                                class="inline-flex items-center px-1.5 py-0.5 rounded-full text-[10px] font-semibold bg-white text-ink-tertiary border border-border">{{ $perms->count() }}</span>
                                        </div>
                                        <div class="grid grid-cols-1 md:grid-cols-2 gap-2">
                                            @foreach ($perms as $permission)
                                                <label
                                                    class="flex items-start gap-2 px-3 py-2 bg-white rounded-xs cursor-pointer hover:bg-brand-tint transition-colors">
                                                    <input
                                                        class="h-4 w-4 mt-0.5 rounded border-border text-brand focus:ring-brand permission-checkbox"
                                                        type="checkbox" name="permissions[]" value="{{ $permission->id }}"
                                                        id="perm_{{ $permission->id }}" @checked(in_array($permission->name, $permissionNames))
                                                        @disabled($isSuper)>
                                                    <span class="flex-1 min-w-0">
                                                        <span
                                                            class="text-sm font-medium text-ink-emphasis block">{{ $permission->title }}</span>
                                                        <code
                                                            class="text-[11px] text-ink-tertiary font-mono">{{ $permission->name }}</code>
                                                    </span>
                                                </label>
                                            @endforeach
                                        </div>
                                ...
                                @empty
                                    <p class="text-ink-tertiary text-sm mb-0">No permissions defined yet. Add them in code
                                        first.</p>
                                @endforelse
                            </div>
                        </div>
                ...
                <div class="p-5">
                    <div class="text-center">
                        ...
                        <div class="relative inline-block" style="width:120px;height:120px;">
                            <svg viewBox="0 0 36 36" width="120" height="120" class="transform -rotate-90">
                                <circle cx="18" cy="18" r="15.915" fill="none" stroke="#E5E7EB"
                                    stroke-width="3" />
                                <circle cx="18" cy="18" r="15.915" fill="none" stroke="#F85606"
                                    stroke-width="3" stroke-dasharray="{{ $pct }} 100" stroke-linecap="round"
                                    transform="rotate(-90 18 18)" />
                            </svg>
                            <div class="absolute inset-0 flex flex-col items-center justify-center">
                                <strong class="font-bold text-ink-emphasis"
                                    style="font-size:1.6rem;line-height:1;">{{ $pct }}%</strong>
                    ...
                    <dl class="text-sm space-y-2 mt-4">
                        ...
                        <div class="flex justify-between">
                            ...
                            <dd class="font-mono font-semibold text-ink-emphasis">{{ $permissionGroups->keys()->count() }}
                            </dd>
                        </div>
                    </dl>
                </div>
            ...
            <section class="bg-white rounded-sm shadow-sm overflow-hidden">
                ...
                <div class="p-5">
                    <ul class="space-y-3 mb-0 text-sm">
                        <li class="flex gap-3">
                            <span
                                class="shrink-0 w-7 h-7 rounded-sm bg-brand-tint text-brand-deep flex items-center justify-center"><i
                                    data-lucide="lock" style="width:14px;height:14px;"></i></span>
                            <div>
                        ...
                        <li class="flex gap-3">
                            <span
                                class="shrink-0 w-7 h-7 rounded-sm bg-info-tint text-feedback-info flex items-center justify-center"><i
                                    data-lucide="users" style="width:14px;height:14px;"></i></span>
                            <div>
                                <p class="mb-0 font-semibold text-ink-emphasis">Group-aware</p>
                                <small class="text-ink-tertiary">Permissions are grouped by their prefix for easier
                                    scanning.</small>
                            </div>
                        </li>
                        <li class="flex gap-3">
                            <span
                                class="shrink-0 w-7 h-7 rounded-sm bg-warning-tint text-feedback-warning flex items-center justify-center"><i
                                    data-lucide="save" style="width:14px;height:14px;"></i></span>
                            <div>
                                <p class="mb-0 font-semibold text-ink-emphasis">Save to apply</p>
                                <small class="text-ink-tertiary">Changes take effect immediately for assigned
                                    admins.</small>
                            </div>
                        </li>
                    </ul>
