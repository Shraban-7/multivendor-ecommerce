@forelse ($cartItems as $item)
    ...
    <tr class="cart-item" id="cart-item-{{ $item->id }}" data-id="{{ $item->id }}" data-variant-id="{{ $item->product_variant_id }}" data-product-id="{{ $item->product_id }}">
        ...
        <td class="text-end align-middle">
            <div class="input-group input-group-sm justify-content-end"
                ...
                <span class="input-group-text">
                    <small class="text-muted">{{ removeZeroFromDecimal($sellingPrice) }}</small>
                </span>

                <input type="number"
                    class="form-control text-end price-input"
                    data-price="{{ removeZeroFromDecimal($sellingPrice) }}"
                    value="{{ removeZeroFromDecimal($discountedPrice) }}"
                    data-id="{{ $item->id }}"
                    min="0" />
            </div>
        </td>
        ...
        <td class="text-end align-middle">
            <button class="btn btn-sm btn-link text-danger p-0 delete-cart-item-btn"
                data-id="{{ $item->id }}"
