...
<div class="container mx-auto px-4 pb-10">
    <div class="mb-4">
        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            ...
            <form method="GET" action="#"
                class="relative w-full md:w-auto max-w-xl rounded-full">
                <input type="text"
                    name="name"
    ...
    <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 md:gap-8">
        ...
        <div class="group bg-white rounded-xl shadow-card hover:shadow-card-hover transition-all duration-300 transform hover:-translate-y-1 overflow-hidden border border-gray-100 relative">
            <div class="h-28 md:h-32 w-full relative overflow-hidden">
                @if($seller->banner_images->count())
                <img src="{{ storage_url($seller->banner_images->first()->image) }}"
                    class="w-full h-full object-cover group-hover:scale-105 transition-transform duration-700">
                @endif
                <div class="absolute inset-0 bg-gradient-to-t from-black/40 to-transparent"></div>

                <span class="absolute top-3 right-3 bg-white/90 text-primary-700 text-[10px] font-bold px-2 py-1 rounded shadow-sm">
                    VERIFIED
                </span>
            </div>
            <div class="px-4 pb-5 relative">
                <div class="absolute -top-8 left-1/2 -translate-x-1/2">
                    <div class="w-16 h-16 md:w-20 md:h-20 rounded-full border-[3px] border-white shadow-md bg-white p-0.5">
                ...
                <div class="pt-10 md:pt-12 text-center">
                    <h3 class="text-sm md:text-lg font-bold text-gray-900 flex justify-center items-center gap-1">
