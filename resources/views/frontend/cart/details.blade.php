        ...
        <div class="lg:col-span-2">
            ...
            <div id="cart-wrapper" class="bg-white rounded-xl shadow-sm p-4 sm:p-6">
                ...
                <div class="py-16 text-center">
                    ...
                    <a href="{{ route('home') }}" class="inline-flex items-center px-5 py-2.5 bg-primary text-white rounded-lg hover:bg-primary-dark transition-colors">
                        Continue Shopping
                    </a>
                </div>
                @else
                ...
                <div class="mb-6 seller-section">
                    ...
                    <div class="flex items-center justify-between p-4 bg-gray-50 rounded-lg mb-4">
                        <div class="flex items-center gap-3">
                            <label for="selectSeller{{ $sellerId }}" class="flex items-center cursor-pointer">
                                <div class="relative">
                                    ...
                                    <div class="w-6 h-6 flex items-center justify-center border-2 border-gray-300 rounded-md peer-checked/seller{{ $sellerId }}:bg-primary peer-checked/seller{{ $sellerId }}:border-primary transition-colors">
                                        <svg class="w-3 h-3 text-white hidden peer-checked/seller{{ $sellerId }}:block" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            ...
                            <div>
                                ...
                                <p class="text-sm text-gray-500">
                                    <span class="seller-count" data-seller-id="{{ $sellerId }}">0</span> of {{ count($cartGroup->flatMap->cart_items) }} items selected
                                </p>
                            </div>
                        </div>
                            ...
                            <div class="flex gap-4">
                                ...
                                <div class="flex items-start pt-2">
                                    <input type="checkbox" id="item{{ $key }}"
                                        ...
                                        data-seller-id="{{ $sellerId }}" />
                                    <label for="item{{ $key }}"
                                        class="flex items-center justify-center w-5 h-5 mt-1 border-2 border-gray-300 rounded cursor-pointer peer-checked/item{{ $key }}:bg-primary peer-checked/item{{ $key }}:border-primary transition-colors">
                                        <svg class="w-3 h-3 text-white hidden peer-checked/item{{ $key }}:block" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M5 13l4 4L19 7" />
                                        </svg>
                                    </label>
                                </div>
                                ...
                                <!-- Item Image -->
                                <div class="flex-shrink-0 w-20 h-20 overflow-hidden rounded-md sm:w-24 sm:h-24">
                                ...
                                <div class="flex flex-col flex-1">
                                    <div class="flex justify-between">
                                        ...
                                        <div class="flex-1 pr-4">
                                            <h2 class="text-sm font-medium text-gray-900 line-clamp-2 sm:text-base">
                                                {{ $item->product->name }}
                                            </h2>
                                    ...
                                    <div class="flex items-center justify-between mt-4">
                                        ...
                                        <div class="quantity-controls" data-id="{{ $item->id }}">
                                            <div class="flex items-center h-9 rounded-lg border border-gray-300 overflow-hidden">
                                                <input type="hidden" class="product-id" value="{{ $key }}">
                                                <input type="hidden" class="variant-sku" value="{{ $item->variant?->sku }}">
            ...
            <div class="sticky top-6">
                    ...
                    <div class="space-y-4">
                        <div class="space-y-3">
                            <div class="flex justify-between">
                                ...
                                <span id="itemsTotal" class="{{ $discount != 0 ? 'line-through text-gray-400' : 'text-gray-900' }}">{{ money($grand_total) }}</span>
                            </div>
                            <div class="flex justify-between">
                                <span class="text-gray-600">Item Discount:</span>
                                <span id="itemDiscount" class="font-medium text-green-600">-{{ money($discount) }}</span>
                            </div>
                        </div>

                        <div class="pt-4 border-t border-dashed border-gray-300">
                            <div class="flex justify-between text-lg font-medium">
                                <span>Estimated Total (<span id="selectedItemsCount">{{ $total_products_count }}</span> Items)</span>
                                <span id="estimatedTotal" class="text-xl text-gray-900">{{ money($sub_total) }}</span>
                            </div>
                        </div>
                    </div>
                ...
                <div class="p-5 mt-5 bg-white border border-gray-200 rounded-xl shadow-sm">
                    <div class="space-y-5 text-sm text-gray-600">
                        ...
                        <div class="flex items-start gap-3">
                            <div class="mt-0.5 text-green-500">
                                <svg width="18" height="20" viewBox="0 0 22 26" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
                        ...
                        <div>
                            ...
                            <p class="mb-2">Shop confidently knowing that if something goes wrong, we've got your back.</p>
                            <a href="#" class="text-primary hover:text-primary-dark font-medium inline-flex items-center gap-1 transition-colors">
