    ...
    <div class="p-6 sm:p-8 lg:p-10 xl:p-12 flex flex-col justify-center">
        ...
        <form action="{{ route('login') }}" method="POST" class="space-y-5">
            ...
            <div>
                ...
                <input required id="email" type="email" name="email" value="{{ old('email') }}" placeholder="Enter your email"
                    class="w-full px-4 py-2.5 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-yellow-400 focus:border-transparent transition-all duration-200 placeholder:text-gray-400" />
            </div>
            ...
            <div class="flex items-center justify-between">
                <label class="flex items-center gap-2 cursor-pointer">
                    <input id="remember" type="checkbox" class="w-4 h-4 text-yellow-500 border-gray-300 rounded focus:ring-2 focus:ring-yellow-400 cursor-pointer" />
                    <span class="text-sm text-gray-700 select-none">Remember me</span>
                </label>
                <a href="{{ route('password.forgot') }}" class="text-sm font-medium text-yellow-600 hover:underline">Forgot password?</a>
            </div>
        ...
        <div class="text-center mb-5">
            <p class="text-sm text-gray-600">
                Don't have an account? <a href="{{ route('signup') }}" class="text-yellow-600 hover:underline">Signup</a>
            </p>
        </div>
